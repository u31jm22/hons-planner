...
# This file was derived from PDDL Parser, available at <https://github.com/pucrs-automated-planning/pddl-parser>.
#
...
class PDDLParser:
    ...
    def scan_tokens(self, filename: str) -> list:
        with open(filename) as f:
            # Remove single line comments
            data = re.sub(r';.*$', '', f.read(), flags=re.MULTILINE).lower()
        return self.tokenize_string(data)

    def tokenize_string(self, data: str) -> list:
        # Tokenize
        stack = []
        list = []
        for t in re.findall(r'[()]|[^\s()]+', data):
            if t == '(':
                stack.append(list)
                list = []
            elif t == ')':
    ...
    def parse_hierarchy(self, group: list, structure, name: str, redefine: bool):
        list = []
        while group:
    ...
    def parse_objects(self, group, name: str):
    ...
    def parse_types(self, group: list):
    ...
    def parse_predicates(self, group: list):
        for pred in group:
    ...
    def frozenset_of_tuples(self, data):
    ...
    def state_to_tuple(self, state) -> tuple:
        return frozenset(tuple(fact) for fact in state)

    # -----------------------------------------------
    # Grounding belongs to the parser
    # -----------------------------------------------
    ...
    def string_to_predicate(self, predicate_str: str) -> tuple:
        group = self.tokenize_string(predicate_str)
        return tuple(group)

    def string_to_formula(self, formula_str: str) -> tuple:
        group = self.tokenize_string(formula_str)
        positive = []
        negative = []
        self.split_predicates(group, positive, negative, '', 'formula')
        positive = self.frozenset_of_tuples(positive)
        negative = self.frozenset_of_tuples(negative)
